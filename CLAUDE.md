...
- **Typography (owner's final call, 2026-07):** ONE typeface — the serif (`--serif`, Iowan Old Style stack) — for everything: UI, her articles, all pages. Typewriter faces (Special Elite, then Courier Prime) and a sans UI were each tried and reverted; the owner chose the serif for readability and looks. Don't reintroduce them. Gold (`--gold`) is the hover color for every interactive element: nav links, chips, pins.
