...
- **Typography (owner's rules, 2026-07):** her words — entry bodies only — are set in Courier Prime (OFL, self-hosted at `public/fonts/`; chosen over Special Elite for readability). Do not spread the typewriter face further. UI and all pages read in the sans (`--sans`, Optima-led stack) for clarity — EXCEPT the About page, which keeps the serif (`.serif-page`). Gold (`--gold`) is the hover color for every interactive element: nav links, chips, pins.
