 ...
 * pins), their chips fall back to the specific label so both stay
 * reachable. Grouped U.S. first (her home ground), then abroad,
...
export function PlacesStrip({ places, onSelect }: PlacesStripProps) {
  ...
  const chipText = (p: Place) => ((pinCounts.get(p.pin) ?? 1) > 1 ? p.label : p.pin)
